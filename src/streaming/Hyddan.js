window.Hyddan = (function (Hyddan) {
    ...
    Hyddan.Player = (function (Player) {
        ...
        var _autoPlay = true,
                ...
                _isPlaying = false,
                _language = null,
                _liveDelayFragmentCount = 4,
                _logToConsole = false,
                _protection = null,
                _source = null,
                _trackProgress = true,
                _usePresentationDelay = false;
        
        var _reset = function () {
            if (!String.isNullOrEmpty(_source)) {
                Player.DashJs.playbackController.unsubscribe(MediaPlayer.dependencies.PlaybackController.eventList.ENAME_PLAYBACK_SEEKING, Player.DashJs.streamController);
                Player.DashJs.playbackController.unsubscribe(MediaPlayer.dependencies.PlaybackController.eventList.ENAME_PLAYBACK_TIME_UPDATED, Player.DashJs.streamController);
                Player.DashJs.playbackController.unsubscribe(MediaPlayer.dependencies.PlaybackController.eventList.ENAME_CAN_PLAY, Player.DashJs.streamController);
                Player.DashJs.playbackController.unsubscribe(MediaPlayer.dependencies.PlaybackController.eventList.ENAME_PLAYBACK_ERROR, Player.DashJs.streamController);

                Player.DashJs.streamController.reset();
                Player.DashJs.abrController.reset();
                Player.DashJs.playbackController.reset();
                Player.DashJs.rulesController.reset();

                Player.DashJs.protectionController = _di.getObject('protectionController');
        ...
        Player.DashJs = (function (DashJs) {
            DashJs.initialize = function () {
                ...
                DashJs.mediaController = _di.getObject('mediaController');
                DashJs.playbackController = _di.getObject('playbackController');
                DashJs.protectionController = _di.getObject('protectionController');
        ...
        Player.initialize = function (context) {
			...
			var _debug = new MediaPlayer.utils.Debug();
            (_di = new dijon.System())
                .mapValue('system', _di)
                .mapOutlet('system')
                .mapValue('eventBus', new MediaPlayer.utils.EventBus())
                .mapOutlet('eventBus')
                .mapValue('debug', _debug)
                .mapOutlet('debug')
				.injectInto(_debug)
                .injectInto(_debug.setup() || (_context = context))
                .injectInto(Player);
        ...
        Player.language = function (language) {
            if (Hyddan.Utils.notNullOrEmpty(language)) {
				var tracks = Hyddan.Player.DashJs.mediaController.getTracksFor('audio', Player.DashJs.streamController ? Player.DashJs.streamController.getActiveStreamInfo() : null);
				for (var track in tracks) {
					if (language === tracks[track].lang) {
						Hyddan.Player.DashJs.mediaController.setTrack(tracks[track]);
						Player.Events.onLanguage(_language = language);
        ...
        Player.position = function (positionInS) {
            ...
            if ('number' === typeof (positionInS) && !isNaN(positionInS)) {
                Player.DashJs.playbackController.seek(positionInS);
                Player.Events.onPosition(positionInS);
        ...
        Player.source = function (source) {
            if (Hyddan.Utils.notNullOrEmpty(source)) {
                ...
                if (Hyddan.Utils.notNullOrEmpty(_element) && Hyddan.Utils.notNullOrEmpty(_source)) {
                    ...
                    Player.DashJs.playbackController.subscribe(MediaPlayer.dependencies.PlaybackController.eventList.ENAME_PLAYBACK_SEEKING, Player.DashJs.streamController);
                    Player.DashJs.playbackController.subscribe(MediaPlayer.dependencies.PlaybackController.eventList.ENAME_PLAYBACK_TIME_UPDATED, Player.DashJs.streamController);
                    Player.DashJs.playbackController.subscribe(MediaPlayer.dependencies.PlaybackController.eventList.ENAME_CAN_PLAY, Player.DashJs.streamController);
                    Player.DashJs.playbackController.subscribe(MediaPlayer.dependencies.PlaybackController.eventList.ENAME_PLAYBACK_ERROR, Player.DashJs.streamController);
                    Player.DashJs.playbackController.setLiveDelayAttributes(_liveDelayFragmentCount, _usePresentationDelay);
                    Player.DashJs.streamController.initialize(_autoPlay, Player.DashJs.protectionController, _protection);
                    Player.DashJs.streamController.load(_source);
					
					Player.DashJs.mediaController.setInitialSettings('audio', {
						lang: _language
					});
